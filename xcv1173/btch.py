...
def addmoviefromfile2():
  with open('mid.list.category') as f:
    content = f.readlines()
    for c in content:
      # print c.strip().split('|')
      pt = c.split('|')[0].strip()
      ...
      print pc
      # url = 'http://xcv1173.appspot.com/ewdfosid93?q=' + urllib.quote(c.strip())
      url = _server_+'/ewdfosid94?q=' + urllib.quote(pt) + '&c=' + urllib.quote(pc)
      result2 = urllib2.urlopen(url)
...
def listbatch2():
  print 'listbatch2 begin'
  
  url = _server_+'/ewdfosid72'
  # url = 'http://localhost:8080/ewdfosid71'
  # result = urlfetch.fetch(url)
  result = urllib2.urlopen(url)
  lll = json.loads(result.read())
